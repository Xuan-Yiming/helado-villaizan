...
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await fetch(`https://graph.facebook.com/v20.0/${page_id}/insights/page_views_total?access_token=${accessToken}`, { cache: 'no-store' });
        const data = await response.json();

        if (!response.ok) {
            throw new Error(`Error al obtener las visitas de Facebook: ${data.error?.message}`);
        }
