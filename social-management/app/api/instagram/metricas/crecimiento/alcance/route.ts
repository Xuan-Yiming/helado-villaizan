...
export async function GET(request: NextRequest) {
    try {
        ...
        if (endDateObj.isSameOrAfter(today)) {
            ...
            //console.warn(`El endDate incluye el día actual. Ajustando el fin a: ${endDate}`);
        }
        ...
        const fetchAllPages = async (urlWithParams: string, collectedData: ReachData[] = []): Promise<ReachData[]> => {
            try {
                const response = await fetch(urlWithParams, { cache: "no-store" });
                const data = await response.json();

                //console.log("Datos recibidos de la API de Instagram (Alcance):", JSON.stringify(data, null, 2));
        ...
        const allReachData = await fetchAllPages(fullUrl);

        // Filtrar y formatear los datos dentro del rango de fechas
        const formattedData = allReachData
            .filter((item: ReachData) => {
        ...
        console.error("Error en el endpoint de alcance:", error);
        return NextResponse.json([{ name: "Sin datos", value: 0 }], { status: 200 });
    }
}
