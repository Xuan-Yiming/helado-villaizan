...
interface ProfileViewsData {
    ...
    value: number;    // Número de visualizaciones del perfil
}
...
export async function GET(request: NextRequest) {
    try {
        ...
        const fetchAllPages = async (urlWithParams: string, collectedData: ProfileViewsData[] = []): Promise<ProfileViewsData[]> => {
            try {
                const response = await fetch(urlWithParams, { cache: "no-store" });
                const data = await response.json();

                //console.log("Datos recibidos de la API de Instagram (Profile Views):", JSON.stringify(data, null, 2));
        ...
        //console.log("Datos enviados al frontend (Profile Views):", JSON.stringify(formattedData, null, 2));
        ...
        console.error("Error en el endpoint de Profile Views:", error);
        return NextResponse.json([{ name: "Sin datos", value: 0 }], { status: 200 });
    }
}
